...
namespace learning {
constexpr char kSaveEndpoint[] = "/api/add";
constexpr char kWinsEndpoint[] = "/wins";
constexpr char kDeleteEndpoint[] = "/delete";
constexpr char kAllCharactersEndpoint[] = "/api/getall";
  ...
  auto SaveCharacterToMongoDb() {
    return [&](served::response &response, const served::request &request) {
      ...
      bool insert_successful = mhandler.AddCharacterToDb(
          request_body["emp_id"].ToString(),
		  request_body["emp_name"].ToString(),
		  request_body["email_id"].ToString(),
          request_body["designation"].ToString(),
		  request_body["department"].ToString(),
